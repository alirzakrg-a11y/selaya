# Audio attributions

SELAYA's own muezzin recordings ship under their own arrangement. The following
sound is used under a Creative Commons licence and is credited here, in the
app's licence page (via `LicenseRegistry`), and in the sound picker:

## Aaqib Azeez (shown in the muezzin picker)

- **Title:** The Adhan – Muslim Call to Prayer
- **Reciter:** Aaqib Azeez
- **Uploaded by (Commons author):** Atcovi
- **Source:** Wikimedia Commons — https://commons.wikimedia.org/wiki/File:The_Adhan_-_Muslim_Call_to_Prayer.mp3
- **Licence:** Creative Commons Attribution-ShareAlike 4.0 International (CC BY-SA 4.0) — https://creativecommons.org/licenses/by-sa/4.0/
- **Changes:** none (used as-is). Bundled as `res/raw/adhan_commons.mp3` and
  `assets/audio/adhan/adhan_commons.mp3`.

Per CC BY-SA 4.0, anyone redistributing this audio must keep this attribution
and license it under the same terms.

## adhan_mecca_full.mp3 (Tam Ezan — Mescid-i Harâm)
- Kaynak: Wikimedia Commons — "Adhan, Great Mosque of Mecca - Jan 21, 2013.webm"
- Lisans: CC BY 3.0 (https://creativecommons.org/licenses/by/3.0/)
- Değişiklik: video içinden ses çıkarıldı, MP3'e dönüştürüldü, ses seviyesi normalize edildi.

## adhan_hassan2_full.mp3 (Tam Ezan — Hassan II Camii, Kazablanka)
- Kaynak: Wikimedia Commons — "Llamada a oración Mezquita Hassan II.wav"
- Lisans: CC BY-SA 4.0 (https://creativecommons.org/licenses/by-sa/4.0/)
- Değişiklik: MP3'e dönüştürüldü, ses seviyesi normalize edildi.

## adhan_mecca_maghrib.mp3 (Akşam Ezanı — Mescid-i Harâm)
- Kaynak: Wikimedia Commons — "Maghrib Adhan at the Masjid al Haram, Mecca - 25 Feb, 2012.webm"
- Lisans: CC BY 3.0 (https://creativecommons.org/licenses/by/3.0/)
- Değişiklik: video içinden ses çıkarıldı, MP3'e dönüştürüldü, ses seviyesi normalize edildi.

## adhan_makkah.mp3 (Mescid-i Harâm · Kısa Ezan, 25 sn)
- Kaynak: Internet Archive — "Makkah Azan" (https://archive.org/details/MakkahAzan_20171111)
- Lisans: CC0 1.0 (kamu malı) — atıf gerekmez
- Değişiklik: ilk 25 sn'ye kırpıldı, sona fade eklendi, ses seviyesi normalize edildi.

## adhan_harak.mp3 (Abdunnâsır Harak · Mısır, 24 sn)
- Okuyan: Abdulnasser Harak (Mısır)
- Kaynak: Internet Archive — "Adhan From Egypt - Abdulnasser Harak" (https://archive.org/details/adhan-from-egypt-abdulnasser-harak)
- Lisans: Public Domain Mark 1.0 — atıf gerekmez
- Değişiklik: ilk 24 sn'ye kırpıldı, sona fade eklendi, ses seviyesi normalize edildi.

## adhan_fakhri.mp3 (Sabah Fahri · Kısa Ezan, 24 sn)
- Okuyan: Sabah Fahri (Suriyeli ses sanatçısı)
- Kaynak: Wikimedia Commons — "Call to prayer by Sabah Fakhry.mp3" (https://commons.wikimedia.org/wiki/File:Call_to_prayer_by_Sabah_Fakhry.mp3)
- Lisans: Public domain (kamu malı) — atıf gerekmez
- Değişiklik: ilk 24 sn'ye kırpıldı, sona fade eklendi, ses seviyesi normalize edildi.

## Telif temizliği (2026-06-11)
Kaynağı belgelenemeyen 7 kısa müezzin klibi (mishary, ahmed_nafees, hafiz_mustafa,
masjid_haram, qari_kareem, sheikh_jamac, salah_mansor) ve ticari albümden geldiği
ID3 etiketiyle tespit edilen karl_jenkins ("The Armed Man", 2001) uygulamadan
tamamen kaldırıldı. notif_chime, ffmpeg ile üretilen kendi sentez çanımızla
değiştirildi. Dağıtılan tüm ezan kayıtları artık yukarıda belgeli CC lisanslıdır.
