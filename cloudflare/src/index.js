    ...
    if (host.startsWith('panel.')) {
      ...
      if (path.startsWith('/api/')) {
        ...
        if (request.method === 'POST' && path === '/api/upload') {
          ...
          // Hikâye sayısı limiti KALDIRILDI (kullanıcı isteği) — istediği kadar
          // hikâye eklenebilir; uygulama da hepsini gösterir (story_rail .take yok).
  ...
  function loadUsers(){
    ...
    api('users').then(function(res){
      ...
      h+=us.map(function(u){
        ...
        return '<div class="item"'+(banned?' style="opacity:.7"':'')+'><div class="ph">'+(banned?'🚫':'👤')+'</div><div class="meta" data-id="'+id+'" style="cursor:pointer" title="Tüm bilgileri gör" onclick="showUserDetail(this.dataset.id)"><b>'+name+ban+'</b><span class="muted">'+em+rumuz+'</span><span class="muted">Kayıt: '+fmtDate(u.created_at)+' · Son aktif: '+fmtDate(u.last_active)+(u.device?' · '+esc(u.device):'')+' · ℹ️ detay için tıkla</span></div>'+
